...
| `AC_SEC_DEC_AUTH_01`     | Admin API uses HTTP `Bearer` authentication                                    | 🟡 Partial         | Bearer authentication is implemented for Create Account and View Accounts, but View Account still uses the legacy `AuthenticatedAdmin` request-extension extractor.                                                                       |
| `AC_SEC_DEC_AUTH_02`     | Authenticated Admin API requests use the HTTP `Authorization` header           | 🟡 Partial         | The new authentication flow reads Bearer credentials from the `Authorization` header for Create Account and View Accounts, but it is not yet wired into every protected Admin API handler.                                                |
...
| `AC_SEC_DEC_AUTH_06`     | Missing authentication returns `401` with a Bearer challenge                   | 🟡 Partial         | Create Account and View Accounts use the new Bearer authentication flow with the required challenge; View Account still uses the legacy request-extension extractor.                                                                      |
| `AC_SEC_DEC_AUTH_07`     | Invalid bearer authentication returns `401` with `invalid_token`               | 🟡 Partial         | The new authentication flow rejects invalid, expired, revoked, expired-session, inactive-account, soft-deleted-account, and unknown tokens for the newly wired endpoints, but it is not yet applied to every protected Admin API handler. |
