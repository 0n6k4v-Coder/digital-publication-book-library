...
| `AC_REQ_NON_FC_04` | Exclude soft-deleted accounts from normal Account operations       | 🟡 Partial         | View Accounts and View Account exclude soft-deleted accounts from normal lookup, but the broader Account operations that must enforce this rule are not implemented yet.              |
...
| `AC_SEC_REQ_NON_FC_08`   | Do not expose credentials through responses, logs, or administrative views     | 🟡 Partial         | Account API responses exclude credential secrets, including View Account, but the broader authentication and account-management surfaces are not complete.                                                               |
...
| `AC_SEC_DEC_AUTH_01`     | Admin API uses HTTP `Bearer` authentication                                    | 🟢 Implemented     | All currently implemented protected Account API endpoints use the Authentication Bearer flow.                                                                                                                            |
| `AC_SEC_DEC_AUTH_02`     | Authenticated Admin API requests use the HTTP `Authorization` header           | 🟢 Implemented     | All currently implemented protected Account API endpoints obtain authentication from the HTTP `Authorization: Bearer <token>` header.                                                                                    |
...
| `AC_SEC_DEC_AUTH_06`     | Missing authentication returns `401` with a Bearer challenge                   | 🟢 Implemented     | All currently implemented protected Account API endpoints use the required missing-credential Bearer challenge.                                                                                                          |
| `AC_SEC_DEC_AUTH_07`     | Invalid bearer authentication returns `401` with `invalid_token`               | 🟢 Implemented     | All currently implemented protected Account API endpoints use the new Bearer validation flow, including invalid, expired, revoked, expired-session, inactive-account, soft-deleted-account, and unknown-token rejection. |
