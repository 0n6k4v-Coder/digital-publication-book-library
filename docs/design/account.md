...
| `AC_REQ_NON_FC_01` | Preserve Account invariants during lifecycle operations            | 🟡 Partial         | Implemented lifecycle operations, including AC_UC_05 last-administrator protection and AC_UC_06 activation-state validation, preserve the defined Account invariants; the remaining lifecycle operations are not implemented yet. |
...
| `AC_REQ_NON_FC_04` | Exclude soft-deleted accounts from normal Account operations       | 🟡 Partial         | View Accounts and View Account exclude soft-deleted accounts from normal lookup, and AC_UC_05 and AC_UC_06 reject soft-deleted targets, but the broader Account operations that must enforce this rule are not implemented yet.   |
...
| `AC_SEC_REQ_NON_FC_08`   | Do not expose credentials through responses, logs, or administrative views     | 🟡 Partial         | Account API responses exclude credential secrets, including View Account, Deactivate Account, and Activate Account, but the broader authentication and account-management surfaces are not complete. |
