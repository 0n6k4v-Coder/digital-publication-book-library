...
| `AC_REQ_NON_FC_04` | Exclude soft-deleted accounts from normal Account operations       | 🟡 Partial         | View Accounts excludes soft-deleted accounts by default and supports explicit `include_deleted=true` for restoration needs, but the broader Account operations that must enforce this rule are not implemented yet. |
...
| `AC_UC_02` | View Accounts       | 🟡 Partial         | The View Accounts use case is implemented, including pagination, status filtering, soft-deleted account exclusion by default, optional inclusion of deleted accounts for restoration needs, response mapping, validation, and `Cache-Control: no-store`, but the production authentication/authorization layer is not implemented; the current `AuthenticatedAdmin` relies on an upstream-provided authenticated identity. |
| `AC_UC_03` | View Account | 🟡 Partial | The View Account use case is implemented, including UUID validation, non-deleted account lookup, response mapping, `404 ACCOUNT_NOT_FOUND`, `Cache-Control: no-store`, and credential-field exclusion, but the production authentication/authorization layer is not implemented; the current `AuthenticatedAdmin` relies on an upstream-provided authenticated identity. |
