...
| `AC_SEC_REQ_NON_FC_08`   | Do not expose credentials through responses, logs, or administrative views     | 🟡 Partial         | Account API responses and the implemented authentication responses do not expose stored credential secrets, but a complete application-wide credential-logging policy is not established. |
...
| `AC_SEC_DEC_AUTH_09`     | Bearer credentials are never exposed in responses or logs                      | 🟡 Partial         | Protected Account API responses do not expose bearer credentials, but the backend does not establish a complete application-wide logging policy.                                          |
