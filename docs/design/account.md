...
| `AC_REQ_FC_07`     | Record the account responsible for creating or updating an account | 🟡 Partial         | Account creation records `created_by` and `updated_by`, but account update operations that maintain `updated_by` are not implemented yet.                                            |
...
| `AC_REQ_FC_09`     | Use email and password authentication                              | 🔴 Not Implemented | The stored email/password credential model exists, but the account email/password authentication flow is not implemented yet.                                                        |
| `AC_REQ_FC_10`     | Use email as the authentication identifier                         | 🔴 Not Implemented | The email credential is stored and normalized, but email/password authentication using it is not implemented yet.                                                                     |
...
| `AC_REQ_FC_16`     | Prevent authentication for inactive accounts                       | 🟡 Partial         | The Admin API Bearer authentication layer rejects inactive accounts, but the account email/password authentication flow is not implemented yet.                                      |
| `AC_REQ_FC_17`     | Prevent authentication for soft-deleted accounts                   | 🟡 Partial         | The Admin API Bearer authentication layer rejects soft-deleted accounts, but the account email/password authentication flow is not implemented yet.                                  |
...
| `AC_REQ_NON_FC_04` | Exclude soft-deleted accounts from normal Account operations       | 🟡 Partial         | View Accounts excludes soft-deleted accounts by default and supports explicit `include_deleted=true`, but the broader Account operations that must enforce this rule are not implemented yet. |
...
| ID                       | Description                                                                    | Status             | Reason                                                                                                                                                                                                 |
| ------------------------ | ------------------------------------------------------------------------------ | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AC_SEC_REQ_FC_01`       | Authenticate using email and password                                          | 🔴 Not Implemented | The account email/password authentication flow is not implemented yet.                                                                                                                                |
...
| `AC_SEC_REQ_NON_FC_08`   | Do not expose credentials through responses, logs, or administrative views     | 🟡 Partial         | Implemented account responses exclude credential secrets, but the broader authentication and account-management surfaces are not complete.                                                            |
...
| `AC_SEC_DEC_AUTH_01`     | Admin API uses HTTP `Bearer` authentication                                    | 🟡 Partial         | Bearer authentication is implemented and fully wired into Create Account, but View Accounts and View Account still use the legacy `AuthenticatedAdmin` request-extension extractor. |
| `AC_SEC_DEC_AUTH_02`     | Authenticated Admin API requests use the HTTP `Authorization` header           | 🟡 Partial         | The new authentication flow reads Bearer credentials from the `Authorization` header, but it is not yet wired into every protected Admin API handler.                                                  |
| `AC_SEC_DEC_AUTH_03`     | Authorization header format is `Authorization: Bearer <token>`                 | 🟢 Implemented     | The Bearer parser accepts the required header format and rejects unsupported or malformed schemes.                                                                                                       |
| `AC_SEC_DEC_AUTH_04`     | Bearer credentials are not provided in URI or request bodies                   | 🟢 Implemented     | The authentication implementation does not read bearer credentials from URI query parameters or request bodies.                                                                                         |
| `AC_SEC_DEC_AUTH_05`     | Admin API protection realm is `admin-api`                                      | 🟢 Implemented     | Implemented authentication errors use the `admin-api` protection realm.                                                                                                                                |
| `AC_SEC_DEC_AUTH_06`     | Missing authentication returns `401` with a Bearer challenge                   | 🟡 Partial         | Create Account uses the new Bearer authentication flow with the required challenge; View Accounts and View Account still use the legacy request-extension extractor.                                  |
| `AC_SEC_DEC_AUTH_07`     | Invalid bearer authentication returns `401` with `invalid_token`               | 🟡 Partial         | The new authentication flow rejects invalid, expired, revoked, expired-session, inactive-account, soft-deleted-account, and unknown tokens, but it is not yet applied to every protected Admin API handler. |
| `AC_SEC_DEC_AUTH_08`     | Missing-credential challenge does not include a Bearer error parameter         | 🟢 Implemented     | The new missing-credential response uses the realm-only Bearer challenge without an `error` parameter.                                                                                                   |
...
| `AC_SEC_DEC_AUTH_10`     | Bearer credentials are transmitted only over HTTPS/TLS                         | 🔴 Not Implemented | The backend does not itself enforce HTTPS/TLS.                                                                                                                                                           |
| `AC_SEC_DEC_AUTH_11`     | Basic, API-key, query-token, and body-token authentication are not accepted    | 🟢 Implemented     | The implemented authentication flow accepts Bearer credentials from the `Authorization` header and does not implement Basic, API-key, query-token, or body-token authentication.                      |
...
| `AC_UC_02` | View Accounts       | 🟡 Partial         | The View Accounts use case is implemented with pagination, status filtering, soft-deleted account exclusion by default, optional inclusion of deleted accounts, response mapping, validation, and `Cache-Control: no-store`, but it still uses the legacy `AuthenticatedAdmin` request-extension extractor instead of the new authentication/authorization flow. |
| `AC_UC_03` | View Account        | 🟡 Partial         | The View Account use case is implemented with UUID validation, non-deleted account lookup, response mapping, `404 ACCOUNT_NOT_FOUND`, `Cache-Control: no-store`, and credential-field exclusion, but it still uses the legacy `AuthenticatedAdmin` request-extension extractor instead of the new authentication/authorization flow. |
...
| `AC_API_02` | View Accounts endpoint       | 🟡 Partial         | The `GET /admin/accounts` endpoint is implemented with pagination, filtering, soft-deleted account handling, validation, `200 OK`, JSON response formatting, and `Cache-Control: no-store`, but it still uses the legacy `AuthenticatedAdmin` request-extension extractor rather than the new authentication/authorization flow. |
| `AC_API_03` | View Account endpoint        | 🟡 Partial         | The `GET /admin/accounts/{id}` endpoint is implemented with UUID validation, `200 OK`, `400 INVALID_ACCOUNT_ID`, `404 ACCOUNT_NOT_FOUND`, JSON/problem responses, `Cache-Control: no-store`, and sensitive-field exclusion, but it still uses the legacy `AuthenticatedAdmin` request-extension extractor rather than the new authentication/authorization flow. |
