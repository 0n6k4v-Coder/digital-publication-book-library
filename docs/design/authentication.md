...
| `AU_REQ_FC_09`     | 🟢 Implemented     | Bearer access-token validation is implemented with token, session, and account-state checks and is used by all currently implemented protected Account API endpoints.                                  |
...
| `AU_SEC_DEC_ACCESS_01`     | 🟢 Implemented     | All currently implemented protected Account API endpoints use HTTP Bearer authentication.                                                                                      |
| `AU_SEC_DEC_ACCESS_02`     | 🟢 Implemented     | All currently implemented protected Account API endpoints obtain bearer credentials from the `Authorization` header.                                                           |
...
| Protected request flow          | 🟢 Implemented     | The protected-request authentication sequence is implemented in the authentication extractor and repository and is used by all currently implemented protected Account API endpoints.      |
...
| `AU_UC_04` | Validate Bearer Authentication | 🟢 Implemented     | Bearer parsing, SHA-256 verifier lookup, token expiry checks, session validation, account-state validation, and `AuthenticatedPrincipal` creation are implemented and used by all currently implemented protected Account API endpoints. |
...
| `AU_CONTRACT_01` | `AuthenticatedPrincipal` | 🟢 Implemented     | `AuthenticatedPrincipal` is implemented with `account_id`, `session_id`, and `authenticated_at`, and is consumed by Authorization for permission decisions on the implemented protected Account API endpoints. |
