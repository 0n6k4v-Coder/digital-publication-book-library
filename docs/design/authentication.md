...
| `AU_REQ_FC_04`     | 🟡 Partial         | Bearer-token validation rejects tokens for inactive accounts, but the email/password authentication flow is not implemented yet.                                                                       |
...
| `AU_REQ_FC_09`     | 🟡 Partial         | Bearer access-token validation is implemented with token, session, and account-state checks, but it is not yet wired into every protected Admin API handler.                                           |
...
| `AU_REQ_FC_12`     | 🟢 Implemented     | `authentication_access_token.expires_at` is persisted and checked during Bearer-token validation.                                                                                                      |
...
| `AU_REQ_FC_19`     | 🟢 Implemented     | Authentication obtains the account authentication state required for protected-request validation from Account-owned database records.                                                                 |
| `AU_REQ_FC_20`     | 🟢 Implemented     | Authentication does not own canonical Account profile data.                                                                                                                                            |
| `AU_REQ_FC_21`     | 🟢 Implemented     | Authentication does not own `account_credentials.password_hash`; password storage remains in the Account domain.                                                                                       |
...
| `AU_REQ_FC_25`     | 🟢 Implemented     | Authorization consumes `AuthenticatedPrincipal` for the `account:create`, `account:view`, and conditional `account:view_deleted` permission decisions.                                                 |
| `AU_REQ_NON_FC_01` | 🟡 Partial         | The implemented authentication path does not persist raw access tokens or log bearer credentials, but an application-wide credential-logging policy and the authentication endpoints are not complete. |
| `AU_REQ_NON_FC_02` | 🟢 Implemented     | Access-token persistence stores the SHA-256 verifier instead of the raw token.                                                                                                                         |
...
| `AU_SEC_DEC_ACCESS_01`     | 🟡 Partial         | Bearer authentication is implemented for Create Account and View Accounts, but View Account still uses the legacy `AuthenticatedAdmin` extractor.                                               |
| `AU_SEC_DEC_ACCESS_02`     | 🟡 Partial         | The authentication extractor accepts bearer credentials from the `Authorization` header for the new authentication flow, but that flow is not yet applied to every protected Admin API handler. |
...
| `AU_SEC_DEC_ACCESS_07`     | 🟡 Partial         | Access-token expiration is persisted and validated, but issuance with the defined 3600-second lifetime is not implemented yet.                                                                  |
| `AU_SEC_DEC_ACCESS_08`     | 🟡 Partial         | The validation flow rejects tokens for revoked sessions, but an explicit session-revocation operation is not implemented yet.                                                                   |
| `AU_SEC_DEC_ACCESS_09`     | 🟢 Implemented     | Authorization is handled separately; access-token validation produces an `AuthenticatedPrincipal` and does not grant roles or permissions.                                                      |
...
| Protected request flow          | 🟡 Partial         | The protected-request authentication sequence is implemented in the authentication extractor and repository. Create Account and View Accounts use the new authentication/authorization flow, while View Account still uses the legacy `AuthenticatedAdmin` extractor. |
...
| `AU_UC_04` | Validate Bearer Authentication | 🟡 Partial         | Bearer parsing, SHA-256 verifier lookup, token expiry checks, session validation, account-state validation, and `AuthenticatedPrincipal` creation are implemented, but View Account still uses the legacy authentication extractor. |
...
| `AU_CONTRACT_01` | `AuthenticatedPrincipal` | 🟢 Implemented     | `AuthenticatedPrincipal` is implemented with `account_id`, `session_id`, and `authenticated_at`, and is consumed by Authorization for the Create Account and View Accounts permission decisions. |
