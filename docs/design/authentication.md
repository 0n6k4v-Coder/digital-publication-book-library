...
| `AU_SEC_DEC_ACCESS_15`     | 🟢 Implemented     | A valid access token remains usable until token, session, or account authentication state becomes invalid; Account deactivation makes the account authentication state invalid for subsequent Bearer validation.                             |
...
| `AU_SEC_DEC_FAILURE_06`    | 🟢 Implemented     | The Bearer-token validation query requires an active account; an account deactivated by AC_UC_05 therefore cannot use its existing Bearer token for subsequent protected requests.                                                           |
...
| `AU_UC_04` | Validate Bearer Authentication | 🟢 Implemented     | Bearer parsing, SHA-256 verifier lookup, token expiry checks, session validation, account-state validation, and `AuthenticatedPrincipal` creation are implemented and used by all currently implemented protected Account API endpoints, including post-deactivation rejection. |
