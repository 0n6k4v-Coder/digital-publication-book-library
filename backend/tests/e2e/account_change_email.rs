...
async fn change_email_end_to_end() {
    ...
    let response = client
        .patch(format!("http://{address}/admin/accounts/{target_id}/email"))
        .bearer_auth(&admin_token)
        .header("content-type", "application/json")
        .json(&serde_json::json!({"email":"  NewAdmin@BÜCHER.DE  "}))
        .send()
        .await
        .expect("send change-email request");
    ...
    let payload: Value = response.json().await.expect("decode account response");
    assert_eq!(payload["id"], target_id.to_string());
    assert_eq!(payload["email"], "NewAdmin@xn--bcher-kva.de");
    assert!(payload.get("password").is_none());
    assert!(payload.get("password_hash").is_none());

    let stored = sqlx::query_scalar::<_, String>(
        "SELECT email FROM account_credentials WHERE account_id = $1",
    )
    .bind(target_id)
    .fetch_one(&pool)
    .await
    .expect("read changed email");

    assert_eq!(stored, "NewAdmin@xn--bcher-kva.de");
...
#[tokio::test]
#[ignore = "requires PostgreSQL and a built backend"]
async fn change_email_authentication_authorization_and_validation_end_to_end() {
    let _database_guard = TEST_DATABASE_LOCK.lock().await;
    let pool = connect_database().await;
    ...
    let (address, server) = start_server(test_router(pool.clone())).await;
    let client = Client::new();
    let uri = format!("http://{address}/admin/accounts/{target_id}/email");
    ...
    let invalid = client
        ...
        .bearer_auth(&admin_token)
        .header("content-type", "application/json")
        .json(&serde_json::json!({"email":"Alice <alice@example.com>"}))
        .send()
        .await
