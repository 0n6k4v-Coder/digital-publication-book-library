...
async fn restore_enforces_authentication_authorization_and_defined_conflicts() {
    ...
    let target_id = seed_account(
        &pool,
        &format!(
            "restore-security-target-{}@example.com",
            Uuid::new_v4()
        ),
        "inactive",
        Some(OffsetDateTime::now_utc()),
    ...
    let response = app(pool.clone())
        .oneshot(restore_request(
            &format!(
                "/admin/accounts/{inactive_not_deleted_id}/restore"
            ),
            Some(&admin_token),
        ))
        .await
    ...
    let response = app(pool.clone())
        .oneshot(restore_request(
            &format!(
                "/admin/accounts/{active_not_deleted_id}/restore"
            ),
            Some(&admin_token),
        ))
        .await
