...
pub fn normalize_email(input: &str) -> Result<NormalizedEmail, EmailValidationError> {
    ...
    let parsed =
        AddrSpec::from_str(trimmed).map_err(|_| EmailValidationError::InvalidFormat)?;

    if parsed.is_literal() {
        return Err(EmailValidationError::InvalidFormat);
    }

    let ascii_domain = domain_to_ascii_strict(parsed.domain())
        .map_err(|_| EmailValidationError::InvalidDomain)?;
    ...
    pub fn from_file(path: &Path) -> Result<Self, PasswordBlocklistLoadError> {
        ...
        for (line_number, line) in reader.lines().enumerate() {
            ...
            for (index, slot) in bytes.iter_mut().enumerate() {
                ...
                *slot =
                    u8::from_str_radix(&hash_text[start..start + 2], 16).map_err(
                        |_| PasswordBlocklistLoadError::Invalid {
                            line: line_number,
...
mod tests {
    ...
    fn hashes_password_with_argon2id() {
        ...
        let password =
            SecretString::from("a secure password with enough length".to_owned());
